...
class RetrieveItem3Response
{
	...
	private function getItemContent($item)
	{
		$retval = '<b>' . $item->RequestKey->Name . '</b> - ' . $item->Status->StatusMsg .
					...
					'<table border="1"><tr><th width="35%">Field</th><th>Value</th></tr>';
		foreach ($item->Fields as $fields)
		{
			foreach ($fields as $field)
			{
				$retval .= '<tr><td>' . $field->n . '</td><td>' . $field->_ . '</td></tr>';
			}
		}
		$retval .= '</table><br/>';
...
$client2 = new SoapClient("http://api.rkd.reuters.com/schemas/wsdl/Quotes/Quotes_1_HttpAndRKDToken.wsdl",
    array('soap_version' => SOAP_1_2, 'trace' => true,                                                        //in order to get the XML response enable trace
            'compression'=> SOAP_COMPRESSION_ACCEPT | SOAP_COMPRESSION_GZIP)              //enable compression
  );
